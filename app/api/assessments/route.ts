...
import { assessments, assessment_results } from "@/db/schema";
...
const ASSESSMENT_TEMPLATES = {
	react: {
		...
		passing_score: 70,
		description:
			"Comprehensive React assessment covering hooks, state management, and best practices",
		questions: [
			{
			...
			},
			{
				...
				question:
					"Complete the useEffect hook to fetch data on component mount:",
	...
	python: {
		...
		passing_score: 75,
		description:
			"Python assessment covering data structures, algorithms, and best practices",
		questions: [
			{
				type: "multiple_choice",
				question:
					"Which of the following is the most efficient way to check if a key exists in a dictionary?",
				options: [
					"key in dict",
					"dict.has_key(key)",
					"try/except KeyError",
...
export async function GET(request: NextRequest) {
	try {
		...
		if (category) {
			whereConditions.push(eq(assessments.skill_category, category));
		}
		if (difficulty) {
			whereConditions.push(eq(assessments.difficulty_level, difficulty));
		}
		...
		const allAssessments =
			whereConditions.length > 0
				...
				: await baseQuery.orderBy(desc(assessments.created_at));
...
export async function POST(request: NextRequest) {
	try {
		...
		if (
			templateKey &&
			ASSESSMENT_TEMPLATES[templateKey as keyof typeof ASSESSMENT_TEMPLATES]
		) {
			// Use predefined template
			assessmentData =
				ASSESSMENT_TEMPLATES[
					templateKey as keyof typeof ASSESSMENT_TEMPLATES
				];
		...
		switch (question.type) {
			case "multiple_choice":
				if (answer.selected === question.correct) {
					...
					questionFeedback = `Incorrect. The correct answer was: ${
			...
			case "coding_challenge":
				...
				questionFeedback = await generateCodeFeedback(
					question,
					answer,
					questionScore
				);
			...
			case "debugging":
				...
				questionFeedback = await generateDebuggingFeedback(
					question,
					answer,
					questionScore
				);
			...
			case "design_question":
				...
				questionFeedback = await generateDesignFeedback(
					question,
					answer,
					questionScore
				);
	...
	try {
		...
		${
			question.function_signature
				? `Expected signature: ${question.function_signature}`
				: ""
		}
		${
			question.expected_keywords
				? `Should include: ${question.expected_keywords.join(", ")}`
				: ""
		}
			...
			messages: [
				{
					role: "system",
					content:
						"You are an expert programming instructor. Grade code fairly and consistently.",
				},
				{
					role: "user",
					content: prompt,
...
async function generateCodeFeedback(
	question: any,
	answer: any,
	score: number
): Promise<string> {
	try {
		const prompt = `
		Provide constructive feedback for this coding answer (scored ${score}/${
			question.points
		}):
			...
			messages: [
				{
					role: "system",
					content:
						"You are a helpful programming mentor. Provide encouraging but honest feedback.",
				},
				{
					role: "user",
					content: prompt,
...
async function gradeDebuggingQuestion(
	question: any,
	answer: any
): Promise<number> {
	...
	const hasCorrectFix =
		answerText.includes("dependency") || answerText.includes("[]");
	return hasCorrectFix ? question.points : Math.floor(question.points * 0.3);
}

async function generateDebuggingFeedback(
	question: any,
	answer: any,
	score: number
): Promise<string> {
...
async function gradeDesignQuestion(
	question: any,
	answer: any
): Promise<number> {
...
async function generateDesignFeedback(
	question: any,
	answer: any,
	score: number
): Promise<string> {
